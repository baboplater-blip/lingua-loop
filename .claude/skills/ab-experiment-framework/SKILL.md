...
- **개입 배선**: `PreRegistration.intervention`(`Intervention` 타입)이 실험군/통제군의 실제 차이를 선언한다. 내장 `practice_order`(실험군=인터리빙·통제군=블록, `core/practice-order.ts`의 `orderByPractice`)를 서빙 계층 `serveItems`가 배정 팔에 따라 적용 — 등록→배정→**차등 서빙**→측정 폐루프가 닫힌다. 순서만 바꾸고 분량은 불변(규칙 1). 새 레버는 `Intervention.kind`를 늘리고 서빙에 배선해 플러그인.
