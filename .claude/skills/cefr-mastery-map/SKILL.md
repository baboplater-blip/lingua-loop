...
- 7개 언어(en·es·zh·ar·sw·ja·hi) **A1~B2** 대칭 완비(콘텐츠·배치·읽기).
- **C1 착수(en 기준 티어)** — `kc.en.inversion`(부정어 도치)·`kc.en.cleft`(분열문) + C1 읽기(`en.read.boredom`)·C1 배치 문항·`cefrFromAbility` 상한 B2→C1. **C1은 생성기가 만들지 않으므로 시드로만**(억지 생성 금지). 다른 언어는 이 패턴을 따라 확장(ja·hi가 파일럿으로 시작했듯). C2·Native+ 는 후속.
