...
- **2026-07-08 (67)** — 공개 리포 첫인상 손질(README 배지·빠른시작·CI). ①README 배지 6종(tests 309 passing·code MIT·content CC-BY·Node≥24·zero-dep·release) + **빠른시작에 clone 단계**(`git clone`→`cd`→`npm run serve`, `npm install` 불필요 명시, ops.html·evolve:all 안내) + 로드맵/라이선스 문구를 배포 완료로 갱신. ②`package.json` version 0.0.1→**0.1.0**(태그 정합) + `repository`·`homepage`·`bugs` 필드. ③**GitHub Actions CI** `.github/workflows/ci.yml` 로컬 준비 완료(모든 push/PR에 `node scripts/gate.mjs`, Node 24, 제로 의존→install 불필요) — ⚠️gh OAuth 토큰에 `workflow` 스코프가 없어 push 보류(`gh auth refresh -h github.com -s workflow` 1회 부여 후 워크플로 push+gate 실동작 배지로 교체 예정). 실증: 온보딩 스모크 5/5(실제 `serve` 부팅→`/`·`/app.js`·`/ops.html`·`/next` 200), 게이트 **309 pass(57파일)**·릴리스 준비도 그린. **새 방문자가 5분 내 clone→실행하도록, 테스트 통과가 배지로 가시화.**
