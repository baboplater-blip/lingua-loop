...
- **2026-07-20 (80)** — 🔁 **FSRS 재적합 영속·적용 — 자기개선 플라이휠 완결**(Phase 6, 규칙 2·1·5·16). 진화 자기개선 3축 중 콘텐츠 생성 ✓·난이도 캘리브레이션 ✓는 영속·서빙됐으나, **FSRS 스케줄러 재적합은 evolve가 A/B 가드레일까지 계산해 `fsrsRefit.deployed`로 판정하고도 그 파라미터를 폐기**(리포트에만 담고 아무도 영속·적용 안 함)하던 마지막 축을 닫음. **코어** `deriveState(events, learnerRef, lang, params?)` — 선택적 FSRS 파라미터를 `nextState`에 전달(없으면 기본값=**하위호환·기존 동작 불변**, 순수·결정적 유지). deriveState 직접 호출자는 `stateOf` 하나뿐이라 변경 완전 contained. **서버** `fsrs.tuned` 시스템 이벤트+`fsrs` 예약 ref(공개 ingest 위조 차단=임의 스케줄 파라미터 주입 방지·규칙 2), `recordFsrsParams`(멱등 append-only)·`tunedFsrsParams`(언어별 최신), **`stateOf`가 튜닝 파라미터를 읽어 스케줄링에 적용**. **evolve-publish**가 `fsrsRefit.deployed`(A/B 리텐션 통과)면 `recordFsrsParams`로 영속 → 모든 학습자 복습 스케줄에 반영. **검증** `core/events-model.test`(deriveState params가 안정성·dueTs 상향·미지정 무회귀)·`server/fsrs-tune.test`(멱등 영속·최신 조회·stateOf 적용·위조 차단·집계 제외)+라이브(evolve-publish FSRS 라인 실행·합성 데이터선 개선 없어 **가드레일이 올바르게 미배포**=규칙 1 안전속성·재실행 멱등). 게이트 **365→370 pass(64파일)**. **이제 evolve 한 사이클이 콘텐츠·난이도·스케줄 3축을 모두 영속·적용 — 성과 개선분만(규칙 1). 자기개선 폐루프 완결.**
- **2026-07-20 (79)** — 🎓 **C1 상급 티어 착수(en 기준) — "0→원어민" 난이도 상한 B2→C1**(Phase 6, 규칙 4·6·11, `cefr-mastery-map`). 북스타는 "0에서 원어민 마스터리까지"인데 콘텐츠가 B2에서 멈춰 있던 상한을 올림. 정확도(규칙 6) 위해 제가 정확히 저술 가능한 **영어**를 기준 티어로(ja·hi가 파일럿으로 시작했듯). **코어** `cefrFromAbility` 확장(능력>3→C1, 2.5→B2 불변 — reading 서빙·placement 레벨 라벨 양쪽 인식). **콘텐츠(데이터·코어 무관)**: en kc-seed에 C1 문법 KC 2종(`kc.en.inversion` 부정어 도치·`kc.en.cleft` 분열문, prereq relative), 각 flashcard+mcq 학습 콘텐츠, C1 논설 지문 `en.read.boredom`(≥200자, 본문에 실제 도치·분열문 → **정직 태깅**·규칙 4), C1 배치 문항 2개(b 2.6·2.9 → CAT 천장 B2→C1). ⚠️**C1은 생성기가 만들지 않으므로 시드로만**(생성 C1=null 유지, 억지 생성 금지). **검증** `core/reading.test`(cefrFromAbility C1)·`server/c1-english.test`(C1 지문 서빙·B2 무회귀·C1 콘텐츠 학습 가능·배치 C1 천장·정직 태깅)+라이브 HTTP 6/6(θ=3.3 학습자→C1 지문 en.read.boredom 서빙·정답 미유출·배치 전부 정답→C1 θ=3.50). 게이트 **361→365 pass(63파일)**. **난이도 스펙트럼이 A1~B2에서 C1(원어민 경로)로 — 다른 언어는 이 패턴 따라 확장.**
- **2026-07-19 (78)** — ⚙️ **무인 캘리브레이션 잡 — 진화 루프 상시 가동 완결**(Phase 6, 규칙 3·5·16). 진화 루프의 캘리브레이션 축(`/calibrate` 커맨드·`engine/calibrate.runCalibration`)이 존재했으나 **무인 스케줄 잡이 없고** 결과가 어디에도 영속·서빙되지 않아, 데이터로 난이도를 보정해도 사라지던 격차를 닫음. **영속 계층** `content.calibrated` 시스템 이벤트+`calibration` 예약 ref(공개 ingest 위조 차단=임의 난이도 주입 방지·규칙 3), `recordCalibration`(캘리브레이션된 아이템만 append-only 기록·**멱등**: 값 안 바뀌면 재기록 안 함)·`calibrationOverlay`(id→최신 난이도/변별도/품질)·`applyCalibrationOverlay`(서빙 뱅크에 오버레이). **서빙·대시보드 반영**: `efficacyReport`·`serveItems`가 오버레이 적용 → Content Health 캘리브레이션 비율이 실제 진척을 보이고 서빙 아이템이 데이터 기반 난이도를 가짐. `RESERVED_REFS`를 단일 소스로 통합(allLearnerEvents/learnerRefs가 공용 로그 5종 일괄 제외). **잡** `scripts/calibrate.mjs`+`npm run calibrate`(수집 응답으로 IRT/ELO 추정·이상 문항 승격 제외·한글 요약), **evolve-publish에 편입**(생성→발행→**캘리브레이션**→스냅샷: 한 사이클이 완전한 플라이휠). **검증** `server/calibrate.test`(영속 멱등·오버레이 최신·efficacyReport 비율 반영·위조 차단·집계 제외)+**라이브**(45응답→3문항 캘리브레이션·Content Health 27.3% 반영·재실행 멱등 스킵 3·evolve-publish 캘리브레이션 라인). OPERATING §2·§4 캘리브레이션 잡 문서화. 게이트 **357→361 pass(62파일)**. **이제 운영자가 evolve:all 하나로 생성·발행·난이도 캘리브레이션·측정을 무인으로 돌린다 — 난이도는 임의값이 아닌 데이터로만(규칙 3).**
- **2026-07-19 (77)** — 🧪 **실험군 개입 실배선 — 통제 실험 폐루프 완성**(Phase 6, 규칙 1·5·16). 76이 사전등록·배정·측정을 만들었으나 배정 팔이 서빙에 아무 영향을 안 줘 "측정 기구"에 머물렀음 → 배정에 따라 **실제로 다르게 서빙**해 등록→배정→**차등 서빙**→측정 폐루프를 닫음. 개입 레버 = **연습 순서**(SLA 근거: 인터리빙 교차연습이 블록연습보다 리텐션·전이에 유리, Rohrer & Taylor). **코어** `core/practice-order.ts` `orderByPractice(items, "interleaved"|"blocked")` — 첫 KC(kc[0]) 기준 라운드로빈(교차) vs 그룹화(블록), **집합 보존·순서만 변경**(분량 불변=참여도 부풀리기 아님·규칙 1)·결정적. `Intervention` 타입(`{kind:"practice_order"}`)을 `PreRegistration.intervention?`에 추가(선택·없으면 관측 비교만). **서버** `practiceOrderFor(store, learnerRef)`(활성 practice_order 실험서 배정 팔→interleaved/blocked, 없으면 null), `serveItems`가 배정 팔에 따라 slice 전 재배열(**미참여 학습자는 기본 순서=무회귀**), `registerExperiment` 개입 전달, `/experiment/assign`이 `practiceOrder`도 반환. **검증**: `core/practice-order.test`(블록/인터리빙/집합보존/빈입력)·`server/experiment-serving.test`(미등록=기본순서 무회귀·실험군 인터리빙·통제군 블록·집합 동일·개입없는 실험은 기본유지)·라이브 HTTP 8/8(등록 개입·배정 practiceOrder·차등 서빙, ⚠️KC당 아이템 1개면 인터리빙=블록=동일순서 정상). OPERATING §6·`ab-experiment-framework` 스킬 갱신. 게이트 **350→357 pass(61파일)**. **이제 운영자가 사전등록 통제 실험을 실제로 돌려 "이 교수법이 정말 학습을 올리는가"를 인과적으로 물을 수 있음(효과 자체는 실학습자가 답=규칙 19).**
- **2026-07-19 (76)** — 🔬 **Phase 6 착수: 사전등록 통제 실험(관측 Gain Score → 인과 증거)**(운영·그로스, 규칙 17·1·5). M3 Gain Score는 *관측값*(성숙·평균회귀·자기선택 혼입)이라 "정말 가르쳤다"의 인과 증거가 아님 — 규칙 17이 요구하는 사전등록 통제 실험 파이프라인을 코어에 배선. **코어** `core/efficacy-experiment.ts`: `PreRegistration`+`validatePreRegistration`(1차 결과를 `gainScore`=학습성과로 강제 → 참여도 실험 등록 거부, 규칙 1), `assignVariant`(FNV-1a+murmur3 finalizer=**결정적·무상태·균형 배정**, 실험 id 혼합으로 실험마다 독립), `compareCohorts`(집단 간 평균차·pooled-SD **Cohen's d**·Welch **95% CI**·`powered`[사전 확정 표본]·`retroactive`[등록이 데이터보다 늦으면 사전등록 무효 경고] → **powered이고 CI가 0 배제할 때만** treatment/control_better, 그 외 no_difference/underpowered). `efficacy.ts`에서 사전/사후 θ 추출을 `gainPairs`로 분리(코호트 비교가 재사용, computeGainScore 동작 보존). **서버** `experiment.registered` 시스템 이벤트+`experiment` 예약 ref(공개 ingest 위조 차단·M1 방어와 정합), `registerExperiment`(멱등·append-only)·`assignForLearner`·`experimentResult`, `POST /experiment`·`GET /experiment/assign`·`GET /experiment/result`. **CLI** `npm run experiment <id>`(인과 주의 병기 한글 요약). **검증**: 코어 합성 코호트(treatment_better/no_difference/underpowered/retroactive 판정·배정 균형 ~50%)·서버 E2E(팔별 gain 심기→treatment_better 검출·위조 차단)·라이브 HTTP 15/15. OPERATING §6 실험 운영·`ab-experiment-framework` 스킬에 구현 SSOT 반영. 게이트 **338→350 pass(59파일)**. ⚠️실험군 **실제 개입 배선은 실험 설계자 몫** — 이 모듈은 사전등록·배정·측정 기구(합성으로 파이프라인 검증). **북스타를 관측에서 인과-가능으로 승격.**
- **2026-07-19 (75)** — 🚀 **v1.0.0 공개 배포**(오너 '배포해' 명시 승인, 규칙 18·14). M1~M5 로드맵 완결 상태를 실제 공개로 컷. 배포 직전 안전검사: 추적 파일 유출 스캔 **0**(`.env`·`.sqlite`·`.pem`·`.key`·`secrets/`·`data/`·`learner-store/` 미추적), 워킹트리 클린, 게이트 **338 pass/57파일** 그린 재확인. 실행: `git push origin main`(로컬 15커밋 `0f5b0b2..f0b9aff`) → annotated 태그 **v1.0.0** 생성·push → `gh release create v1.0.0`(승인 대기 문구 제거한 한/영 노트, draft/prerelease 아님) → GitHub Actions 게이트 CI **success**(15s). 검증: 원격 `origin/main..main` **0 ahead**·릴리스 공개 확인·<https://github.com/baboplater-blip/lingua-loop/releases/tag/v1.0.0>. 문서 동기화: CHANGELOG `[1.0.0]` `Unreleased`→**2026-07-19 릴리스 확정**, `RELEASE_NOTES_v1.0.0` "승인 대기"→"배포됨", status/plan 배포 반영·다음 액션을 **Phase 6 운영·그로스**(효능 연구 정례화·상시 진화·언어팩 확장)로 전환. **v1.0.0 = 적대적 검증 통과 + 효능 파이프라인 + 무인 운영 + 커뮤니티 인수 가능 상태로 공개 — "디버깅→최종 완결" 로드맵 종결.**
- **2026-07-19 (74)** — 🏁 **M4 운영 정례화 + M5 v1.0.0 컷 준비**(로드맵 완결, 규칙 18). **M4**: 무인 폐루프를 라이브 리허설 — `evolve:publish` 1회차 문항13·지문7 발행, 2회차 재발행 0(멱등), 사이클마다 효능 스냅샷. **롤백 리허설**(발행 문항에 나쁜 학습효과 25응답 주입→`publishedBank` 자동 제외·발행 이력은 append-only 보존) 4/4. OPERATING §7에 리허설 결과 기록(스케줄러 실설치는 운영자 시스템 영역). **M5**: CHANGELOG를 `[0.1.0] 릴리스됨` + `[1.0.0] 배포 승인 대기`로 재구성(Security·Added·Changed·Fixed 카테고리로 스윕 전체 정리), `package.json` version 0.1.0→**1.0.0**(코어 API 변경=estimateAbility se nullable·EfficacySnapshot gain 필드 → semver 메이저), `docs/RELEASE_NOTES_v1.0.0.md` 한/영 초안. `npm run release -- 1.0.0` **dry-run 그린**(게이트·카드 8종·라이선스·시크릿·self-host 스모크). 게이트 **338 pass(57파일)** 불변. ⚠️**태그·push·공개는 '배포해' 명시 승인 시에만**(규칙 18) — 준비만 완료. **v1.0.0 로드맵 M1~M5 전부 완결(디버깅→백로그→효능→운영→릴리스 준비).**
- **2026-07-19 (73)** — 📈 **M3 Gain Score 효능 파이프라인**(북스타 goal.md §3, 규칙 1·17). v1.0.0 로드맵 M3 — "도구가 실제로 가르치는가"의 핵심 증거를 코어에 배선. `core/efficacy.computeGainScore(events)`: `assessment.item`(θ 추정) 이벤트에서 학습자·스킬별 **첫(사전=배치)↔최신(사후=재평가) θ**를 뽑아 평균 상승·**관측 효과크기 Cohen's d**(pooled SD) 산출. **재평가(2회 이상) 없으면 표본 0** → 배치 1회로 가짜 개선 안 만듦. `EfficacyReport.gain`(computeEfficacy 통합)·`EfficacySnapshot.gainEffectSize/gainN`·`trendSummary` 델타 편입 → `/efficacy`·ops 대시보드 "Gain Score" 카드·CLI에 노출, **전부 인과 주의 문구 병기**(규칙 17: 관측값일 뿐, 인과는 사전등록 A/B·통제군 필요, n 항상 표시). **합성 코호트 검증**: 학습군 d≥0.4·통제군 ~0·학습군>통제군(측정 가능성 입증). `core/efficacy.test`(Gain Score 학습/통제·재평가 없으면 미산출)·`server/efficacy.test`·`ops.test`(Gain 카드·인과 문구). 실증: 라이브 스모크 8/8(`/efficacy` gain d≥0.4·스냅샷 gainEffectSize·단일평가 제외). OPERATING.md §6 Gain Score 채우는 법(배치 재평가)·인과 주의. 게이트 **335→338 pass(57파일)**. **실학습자 효능은 커뮤니티/배포 영역(goal §5-5) — 이 마일스톤은 측정 파이프라인+합성 증명.**
- **2026-07-19 (72)** — 📚 **M2 백로그 소진**(good first issue 3건, 규칙 4·11). 디버깅 스윕(71) 후 v1.0.0 로드맵 M2 실행. **#2 생성 지문 상위 문법 정직 태깅**: `multilingual-reading.upperGrammarKcs(lang,text,level)` — **B1/B2 등급에서 본문에 실제 마커가 있는 상위 문법 KC만** 크레딧(규칙 4, 없는 문법 태깅 금지). zh B2 了→aspect_le(B1은 了 없어 미태깅·得는 독립토큰만이라 觉得 오탐 0)·ja B1 て형/B2 て형+가능형(でき)·ar B2 과거(غيّر)·hi B1/B2 후치사(में)·sw는 -li- 과거가 템플릿에 없어 정직하게 미태깅. `answerReading`이 이미 지문 kc 전체 크레딧 → 태깅 정확도만 관건. **#3 주제 다양화**: 생성기에 `topics(kc,level)` + `generate(spec.topic)` + 엔진 `generateGradedReadings(...,{includeTopics})` **옵트인**(기본 4등급/개수 불변 → 기존 evolve/reading 테스트 안전) — zh A2 "在车站"·B1 "一次旅行" 대체 주제(주제 접미사 id로 공존·멱등·클릭사전·마지막 주관식 산출, B1 trip은 玩得→de_complement 근거 태깅). **#1 언어팩 가이드**: `docs/ADD_LANGUAGE.md`(최소 3파일 복붙 예시·검증 루프·규칙 체크, CONTRIBUTING·README 링크) — 이슈는 help wanted 유지. `multilingual-reading.test`(상위 문법 정확 태깅+근거 마커·주제 다양화 유효/고유id/산출)·`engine/reading.test`(includeTopics 6편·멱등). 게이트 **333→335 pass(57파일)**. **읽기가 상급 문법까지 정직하게 측정, 주제 반복 감소 능력 확보, 새 기여자 온보딩 문서 완결.**
- **2026-07-19 (71)** — 🛡️ **적대적 디버깅 스윕 → v1.0.0 준비**(설계 `docs/design/final-completion.md`, 규칙 15·16). 페이블 자율 진행으로 결함 사냥 4라운드(R1 정적·적대적 코드 리뷰[core·server·adapters 5개 병렬 에이전트]·R2 7언어 라이브 E2E 매트릭스 180/0·R3 내결함·JSONL↔SQLite 동등성·R4 보안 최종 감사) 실행 → 확증된 결함을 **재현→수정→회귀 테스트→게이트 편입**으로 전부 닫음. **수정(19건, 5커밋)**: ①CRITICAL 게이트 우회(공개 `/events`가 예약 로그 community/published/efficacy·시스템 이벤트 타입 위조 쓰기 가능 → `ingest` 가드, `recordEfficacy` 직접 append) ②CRITICAL 저장형 XSS(app.js가 커뮤니티·읽기 콘텐츠를 innerHTML에 미이스케이프 → `esc()` 전면 적용) ③kc 비배열 영구 오염 DoS(ingest·content-gate 검증+deriveState/efficacy 심층 가드+kc 중복제거) ④커뮤니티 중복투표(1인1표 dedup, Sybil 완화) ⑤efficacy 로그 삭제 보호 ⑥잘린 JSONL 라인 부팅 크래시(자가복구·원자적 재기록) ⑦요청 본문 512KB 캡→413 ⑧이벤트 deep freeze(규칙 5, kc·payload까지) ⑨BKT 첫 오답 숙달 오상승 보정(첫정답 0.25·2정답 0.70 불변) ⑩placement θ/se 클램프 코어 이전(발산 방지·se null) ⑪fsrs 음수 경과일 클램프 ⑫validateReading CEFR 레벨 검증 ⑬calibration 이상문항 승격 제외 ⑭difficultyFit 방향 대칭(too_easy 부당우대 제거) ⑮stressScore/pronunciation 크래시 가드 ⑯withSafety 정규화(전각·제로폭·구두점 우회 차단)+역할극 오탐 제거+history 검사 ⑰CJK 산출 정답 붙여쓰기 허용 ⑱NaN ts 가드 ⑲배열 검증(history/responses/choice). **문서**: SECURITY.md 인증·IDOR·다중사용자 위협모델 명문화, `docs/KNOWN_LIMITATIONS.md` 신설(튜터 다절 과잉교정=오프라인 폴백 한계·자가채점·이벤트 정렬 등 정직 기록, "오탐 0"=단일절 기준으로 정정). 게이트 **313→332 pass(57파일)**·R2 180/0·라이브 보안 스모크 8/8·릴리스 준비도 그린. **핵심 뿌리="수집/서빙 경계에서 클라이언트 입력을 재검증 없이 신뢰" 차단 — 데이터 무결·규칙 4·5·6 방어 완결.**
- **2026-07-08 (70)** — 기여 인프라(이슈/PR 템플릿·라벨·good first issue, 규칙 1·9·11). 공개 리포에 외부 기여를 받을 채비 — CI(자동 게이트)와 짝. `.github/ISSUE_TEMPLATE/`: config(빈 이슈 차단·보안은 advisory·질문은 Discussions) + 이슈 폼 3종(🐛 버그[영역·환경·PII 미포함 체크], 🌍 언어팩[문자체계·포함 데이터·코어 0줄 체크·good first issue 라벨], 💡 콘텐츠/기능[성과 근거 필수·다크패턴 금지 체크]). `.github/PULL_REQUEST_TEMPLATE.md`: rules 체크리스트(게이트 그린·성과가 진실·다크패턴 없음·append-only·미검증 미노출·자가호스팅·erasable TS·DCO). CONTRIBUTING §2.5 처음 기여 흐름(이슈→Discussions→작은 PR→CI 게이트→머지)·라벨 taxonomy. 라벨 생성(language-pack·content·core·evolution·help wanted + 기본 good first issue). 전부 한/영 병기·규칙 인용. 게이트 **313 pass(57파일)** 불변(문서·yaml). **기여→자동 검증→머지 흐름 완성.**
- **2026-07-08 (69)** — 문법 스킬 읽기 연결(읽기 채점이 어휘뿐 아니라 문법 숙달에도 기여, 규칙 1·11). 메커니즘은 이미 `answerReading`이 `p.kc` **전체** 크레딧 → 격차는 KC 태깅. 구조적 결함(비라틴 5개 언어의 A2 문법 KC 부재)을 체계적으로 수정: **A2 문법 KC 5종 신설**(`kc.zh.basic_sentence`·`kc.ar.present_tense`·`kc.sw.present_tense`·`kc.ja.masu_form`·`kc.hi.present_habitual`, en·es는 이미 A2 문법 보유 → 7개 언어 대칭) + 각 flashcard+mcq content-seed(학습 가능) + **A2 시드 지문 5개에 정직 태깅**(각 지문이 실제로 그 문법을 담음: zh 어순·ar/sw 현재형·ja ます형·hi 현재습관). **생성기 systemic 배선**(다국어·en·es 생성기가 등급 vocab.core 지문에 기초 문법 KC를 함께 태깅 → 새 생성 지문도 문법 크레딧, 상위 문법은 템플릿마다 유무 달라 미부착=규칙 4). `reading.test`(모든 지문 문법 KC 보유 7언어·신설 A2 KC가 2회 정답으로 숙달 도달 5언어)·`multilingual-reading.test`(생성 지문 어휘+문법)·`reading-gen.test`(등급 지문 present_simple 크레딧). 실증: 라이브 HTTP 13/13(`/reading/answer` 2회→`/state` 문법 숙달 0.70, 신설 KC 콘텐츠). 게이트 **313 pass(57파일)**·릴리스 준비도 그린. **읽기 한 번이 어휘+문법을 함께 측정 — 마스터리 스택·커버리지 정밀도 상향.**
- **2026-07-08 (68)** — 추이 스파크라인(ops 대시보드에 효능 스냅샷 시퀀스를 SVG 꺾은선으로, 규칙 1). `/efficacy/history`가 이미 반환하던 `snapshots[]` 전체 시퀀스를 ops.js가 소비하도록 — `ops.js` 순수 `sparkline(vals, better)`(제로 의존 SVG path, null 은 x축 위치 보존하며 건너뜀, 첫↔최신 방향으로 선 색: 초록=개선·빨강=악화·파랑=평탄, better=false면 하락이 개선) + 추이 카드 4지표(전체 정확도·복습 정확도·숙달까지 응답[하락=개선]·숙달 KC) 각각에 스파크라인 슬롯(`sp-acc`·`sp-review`·`sp-ttm`·`sp-kcs`)·`.tv` flex 레이아웃·범례. `ops.test`(슬롯 4개·sparkline 함수·`.snapshots` 소비·SVG path·innerHTML·TTM better=false). 실증: 라이브 스모크 9/9(스냅샷 3개 누적→kcsMastered 0→2 상승, 정확도/숙달 SVG path 생성, 개선색, 빈/단일 시퀀스 안전). 게이트 **310 pass(57파일)**·릴리스 준비도 그린. **운영자가 "진화가 학습을 개선하는가"를 첫↔최신 델타 숫자를 넘어 시각 추세로 확인.**
- **2026-07-08 (67)** — 공개 리포 첫인상 손질(README 배지·빠른시작·CI). ①README 배지 6종(tests 309 passing·code MIT·content CC-BY·Node≥24·zero-dep·release) + **빠른시작에 clone 단계**(`git clone`→`cd`→`npm run serve`, `npm install` 불필요 명시, ops.html·evolve:all 안내) + 로드맵/라이선스 문구를 배포 완료로 갱신. ②`package.json` version 0.0.1→**0.1.0**(태그 정합) + `repository`·`homepage`·`bugs` 필드. ③**GitHub Actions CI** `.github/workflows/ci.yml`(모든 push/PR에 `node scripts/gate.mjs`, Node 24, 제로 의존→install 불필요) — gh 토큰에 `workflow` 스코프 부여(디바이스 플로우 1회 승인) 후 push 완료, README 배지도 실동작 gate 배지로 교체. 실증: 온보딩 스모크 5/5(실제 `serve` 부팅→`/`·`/app.js`·`/ops.html`·`/next` 200), 게이트 **309 pass(57파일)**·릴리스 준비도 그린. **새 방문자가 5분 내 clone→실행하도록, 테스트 통과가 배지로 가시화.**
