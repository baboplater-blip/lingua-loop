...
[![zero-dependency](https://img.shields.io/badge/deps-0-lightgrey.svg)](package.json)
[![release](https://img.shields.io/github/v/release/baboplater-blip/lingua-loop?sort=semver)](https://github.com/baboplater-blip/lingua-loop/releases)
...
**Node.js 24 하나면 됩니다** — `npm install` 불필요, 외부 API·빌드·`node_modules` 없음(규칙 13).

```bash
git clone https://github.com/baboplater-blip/lingua-loop.git
cd lingua-loop
npm run serve     # → http://localhost:8787  학습 웹앱 + 데이터 백엔드 (재시작해도 학습 유지)
```

브라우저에서 `http://localhost:8787` 을 열면 학습이 시작됩니다. 운영자 효능 대시보드는 `http://localhost:8787/ops.html`.
...
npm run evolve:all  # 무인 진화 폐루프 1사이클(전 언어) — 운영 루프: docs/OPERATING.md
```

자세히: [`docs/SELF_HOSTING.md`](docs/SELF_HOSTING.md) · 운영 루프: [`docs/OPERATING.md`](docs/OPERATING.md) · 기여: [`CONTRIBUTING.md`](CONTRIBUTING.md) · 새 언어 추가: [`docs/ADD_LANGUAGE.md`](docs/ADD_LANGUAGE.md) · 투명성: [`docs/DATA_CARD.md`](docs/DATA_CARD.md) · [`docs/MODEL_CARD.md`](docs/MODEL_CARD.md) · 보안·인증: [`SECURITY.md`](SECURITY.md) · 알려진 한계: [`docs/KNOWN_LIMITATIONS.md`](docs/KNOWN_LIMITATIONS.md).
...
Phase 0 하네스 ✅ → 1 마스터리 스택 MVP ✅(영속 포함) → 2 진화 루프 ✅ → 3 몰입·튜터 ✅ → 4 다국어 확장 ✅ → 5 OSS 공개 ✅([**v0.1.0 공개 배포**](https://github.com/baboplater-blip/lingua-loop/releases/tag/v0.1.0)) → 6 운영·효능 연구(진행). 상세: [`plan.md`](plan.md) · [`status.md`](status.md).
...
전체 명세: [`LICENSES.md`](LICENSES.md). 공개 배포(태그·push)는 오너의 명시 승인 시에만 진행(규칙 18) — [v0.1.0](https://github.com/baboplater-blip/lingua-loop/releases/tag/v0.1.0) 공개됨.
