...
test("주제 다양화(#3): 대체 주제가 유효한 지문·고유 id·산출 문항으로 생성된다", () => {
  const g = new MultilingualReadingGenerator("zh");
  // zh A2 는 기본 + 대체 주제(station), B1 은 기본 + trip
  assert.deepEqual(g.topics("kc.zh.vocab.core", "A2"), ["", "station"], "A2 주제 목록");
  assert.deepEqual(g.topics("kc.zh.vocab.core", "B1"), ["", "trip"], "B1 주제 목록");
  assert.deepEqual(g.topics("kc.zh.vocab.core", "A1"), [""], "A1 은 기본 주제만");
  // 대체 주제 생성: 유효·고유 id·기본과 다른 본문·마지막 문항 주관식(산출)
  const base = g.generate({ lang: "zh", kc: "kc.zh.vocab.core", level: "A2" })!;
  const station = g.generate({ lang: "zh", kc: "kc.zh.vocab.core", level: "A2", topic: "station" })!;
  assert.ok(validateReading(station), "대체 주제 지문 validateReading 통과(규칙 4)");
  assert.notEqual(station.id, base.id, "id 가 기본과 다름(공존)");
  assert.ok(station.id.includes(".station."), "id 에 주제 슬러그");
  assert.notEqual(station.text, base.text, "본문이 기본 주제와 다름(주제 다양화)");
  assert.ok(Object.keys(station.glossary).length > 0, "클릭 사전 포함");
  assert.equal(station.questions!.at(-1)!.options, undefined, "마지막 문항은 주관식(산출)");
  // 미지정 topic 은 기본 주제(하위호환)
  assert.equal(g.generate({ lang: "zh", kc: "kc.zh.vocab.core", level: "A2" })!.id, base.id, "topic 미지정=기본");
  // 근거 있는 상위 문법도 유지 — B1 trip 본문의 "玩 得"(정도보어) → de_complement
  const trip = g.generate({ lang: "zh", kc: "kc.zh.vocab.core", level: "B1", topic: "trip" })!;
  assert.ok(trip.kc.includes("kc.zh.de_complement"), "trip 본문의 玩 得 → 정도보어 KC(근거 기반)");
});
